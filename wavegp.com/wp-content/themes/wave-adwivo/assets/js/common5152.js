...
(function( $ ) {
    ...
    $( window ).on( "load", function() {
		if (!$('body').hasClass('elementor-editor-active')) {
			 $('.parallax-section').each(function (index, element) {
				var image = $(element).attr('data-image-src');
				if ($(window).width() > 768 ) {
					$(element).parallax({
						imageSrc: image,
					}).addClass('init');
    ...
    $('.job-item .job-apply-btn').on('click', function (e) {
        $('#js-job-title').val( $(this).closest('.job-item').find('.job-item__title').text() );
    })
